  ...
  return (
    <View style={s.card}>
            ...
            <View key={bev.id} style={[s.row, idx === beverages.length - 1 && { borderBottomWidth: 0 }]}>
              ...
              <View style={{ flexDirection: "row", alignItems: "center", gap: 10 }}>
                <Pressable
                  ...
                  style={[s.toggleButton, bev.enabled ? s.toggleButtonOn : s.toggleButtonOff]}
                >
                  <Text style={[s.toggleText, { color: bev.enabled ? colors.white : colors.slateZinc }]}>
                    {togglingId === bev.id ? "…" : bev.enabled ? "Enabled" : "Disabled"}
                  </Text>
                </Pressable>
                <Pressable disabled={deletingId === bev.id} onPress={() => handleDelete(bev)}>
      ...
      <View style={s.addForm}>
        <TextInput style={s.input} value={name} onChangeText={setName} placeholder="Beverage name (e.g. Hot Chocolate)" placeholderTextColor={colors.softZinc} />
        ...
        <View style={s.iconGrid}>
          {BEVERAGE_ICON_NAMES.map((iconName) => {
            ...
            return (
              <Pressable key={iconName} onPress={() => setIcon(iconName)} style={[s.iconOption, selected && s.iconOptionSelected]}>
...
const styles = (colors: ColorRamp) =>
  StyleSheet.create({
    ...
    toggleButton: { borderRadius: 6, paddingHorizontal: 8, paddingVertical: 5, borderWidth: 1 },
    toggleButtonOn: { backgroundColor: colors.ink, borderColor: colors.ink },
    toggleButtonOff: { backgroundColor: colors.white, borderColor: colors.hairlineZinc },
    ...
    fieldLabel: { fontSize: 9, fontWeight: "700", letterSpacing: 0.5, textTransform: "uppercase", color: colors.softZinc },
    iconGrid: { flexDirection: "row", flexWrap: "wrap", gap: 6 },
    iconOption: { width: 32, height: 32, borderRadius: 8, borderWidth: 1, borderColor: colors.dividerZinc, alignItems: "center", justifyContent: "center", backgroundColor: colors.white },
