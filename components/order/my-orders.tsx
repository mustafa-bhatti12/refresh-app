  ...
  return (
    <View style={{ flexDirection: "row", alignItems: "center", gap: 8, marginTop: 6 }}>
      <View style={{ flexDirection: "row", alignItems: "center", gap: 4, backgroundColor: colors.surfaceZinc, borderWidth: 1, borderColor: colors.dividerZinc, borderRadius: 6, paddingHorizontal: 6, paddingVertical: 2 }}>
        ...
        <Text style={{ fontSize: 10, fontWeight: "700", color: colors.midZinc }}>Edit window: {timeLeft}s</Text>
      </View>
      <Pressable onPress={onEditClick} style={{ flexDirection: "row", alignItems: "center", gap: 3, borderWidth: 1, borderColor: colors.hairlineZinc, borderRadius: 6, paddingHorizontal: 8, paddingVertical: 2, backgroundColor: colors.white }}>
        <Text style={{ fontSize: 10, fontWeight: "700", color: colors.ink }}>Edit Order</Text>
...
export function MyOrdersPanel({
  ...
  return (
    <View style={s.card}>
      ...
      {lastOrder && (
        <Pressable disabled={isReordering} onPress={() => onReorder(lastOrder)} style={s.reorderRow}>
                    ...
                    <View style={{ alignItems: "flex-end", gap: 8 }}>
                      ...
                      {order.status === "Delivered" && order.feedbackComments !== "__NOT_FOUND__" && (
                        hasReview ? (
                          ...
                          <Pressable onPress={() => onRateOrder(order.id)}>
...
const styles = (colors: ColorRamp) =>
  StyleSheet.create({
    ...
    reorderRow: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", gap: 8, borderRadius: 8, borderWidth: 1, borderColor: colors.dividerZinc, backgroundColor: colors.surfaceZinc, paddingHorizontal: 12, paddingVertical: 10, marginBottom: 12 },
    reorderText: { fontSize: 12, fontWeight: "600", color: colors.slateZinc, flexShrink: 1 },
    quickOrderLabel: { fontSize: 9, fontWeight: "700", letterSpacing: 0.6, textTransform: "uppercase", color: colors.softZinc },
    ...
    orderRow: { paddingVertical: 14, borderBottomWidth: 1, borderBottomColor: colors.dividerZinc },
    ...
    metaTextSmall: { fontSize: 10, color: colors.softZinc, marginTop: 2 },
    ...
    queuePillText: { fontSize: 10, fontWeight: "700", color: colors.midZinc },
    cancelLink: { fontSize: 10, fontWeight: "700", color: colors.quietZinc },
