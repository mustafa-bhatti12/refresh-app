    ...
    <View style={s.container}>
      ...
      <View style={s.chipRow}>
        {availableBeverages.map((b) => (
          <Pressable key={b.id} onPress={() => setDrink(b.name)} style={[s.chip, drink === b.name && s.chipActive]}>
      ...
      <View style={s.chipRow}>
        {sugarOptions.map((opt) => (
          <Pressable key={opt} onPress={() => setSugar(opt)} style={[s.chip, sugar === opt && s.chipActive]}>
      ...
      <View style={s.chipRow}>
        {floors.map((f) => (
          <Pressable key={f} onPress={() => setFloor(f)} style={[s.chip, floor === f && s.chipActive]}>
      ...
      <View style={s.actions}>
        <Pressable disabled={saving} onPress={onCancel} style={s.cancelButton}>
          <Text style={s.cancelText}>Cancel</Text>
        </Pressable>
        <Pressable disabled={saving} onPress={handleSave} style={s.saveButton}>
...
const styles = (colors: ColorRamp) =>
  StyleSheet.create({
    ...
    label: { fontSize: 9, fontWeight: "700", letterSpacing: 0.6, textTransform: "uppercase", color: colors.quietZinc, marginTop: 4 },
    chipRow: { flexDirection: "row", flexWrap: "wrap", gap: 6 },
    chip: { borderWidth: 1, borderColor: colors.hairlineZinc, borderRadius: 6, paddingHorizontal: 8, paddingVertical: 4, backgroundColor: colors.white },
